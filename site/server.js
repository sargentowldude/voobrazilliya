...
const nav = () => `<header class="site-header"><a class="wordmark" href="/" aria-label="${brandName}"><img src="/brand-logo.svg" alt=""></a><button class="menu-button" type="button" aria-expanded="false">МЕНЮ +</button><nav class="site-menu" aria-label="Основная навигация"><a href="/animatory/">Аниматоры</a><a href="/detskiy-den-rozhdeniya/">День рождения</a><a href="/show/">Шоу</a><a href="/spektakli/">Спектакли</a><a href="/afisha/">Афиша</a></nav></header>`;
...
const eventCards = events => `<div class="event-grid">${events.filter(visible).map((event, index) => `<article class="poster-card poster-card--${['yellow','pink','red'][index % 3]}">${event.image ? `<div class="poster-card__image ${event.imageFit === 'poster' ? 'poster-card__image--poster' : ''}">${image(event)}</div>` : ''}<span class="mono-tag">${escapeHtml(event.category || 'Афиша')}${event.date ? ` · ${escapeHtml(formatEventDate(event.date))}` : ''}</span><h3>${escapeHtml(event.title)}</h3>${event.description ? `<p>${escapeHtml(event.description)}</p>` : ''}${event.buttonUrl ? `<a class="poster-card__cta" href="${escapeAttr(event.buttonUrl)}"><span>${escapeHtml(event.buttonLabel || 'Открыть')}</span><b>↗</b></a>` : `<a class="poster-card__cta" href="/afisha/${escapeAttr(event.slug)}/"><span>${escapeHtml(event.buttonLabel || 'Открыть афишу')}</span><b>↗</b></a>`}</article>`).join('')}</div>`;
...
const renderBirthday = async () => {
  ...
  const birthdayFormatCard = ({ href, title, description, age, duration, guests, price, includes, variant, photo }) => `<a class="birthday-format-card birthday-format-card--${escapeAttr(variant)}" href="${escapeAttr(href)}"><div class="birthday-format-card__media">${image(photo)}</div><div class="birthday-format-card__copy"><span class="mono-tag">Формат праздника</span><h3>${escapeHtml(title)}</h3><p>${escapeHtml(description)}</p></div><dl class="birthday-format-card__facts"><div><dt>Возраст</dt><dd>${escapeHtml(age)}</dd></div><div><dt>Длительность</dt><dd>${escapeHtml(duration)}</dd></div><div><dt>Гостей</dt><dd>${escapeHtml(guests)}</dd></div><div><dt>Стоимость</dt><dd>${escapeHtml(price)}</dd></div></dl><div class="birthday-format-card__includes"><small>В программе</small><strong>${escapeHtml(includes)}</strong></div><span class="birthday-format-card__cta">ВЫБРАТЬ <b>↗</b></span></a>`;
  const serviceCards = [
